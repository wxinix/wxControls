...
type
  ...
  strict private
  class var
    FScaledColorRectWidth: Integer;
    FMinimumPlusSignWidth: Integer;
    FMaximumGutterWidth: Integer;
    FEditors: TDictionary<Pointer, TComponentClass>;
  private
    {$REGION 'class constructor and destructor'}
    class constructor Create;
    class destructor Destroy;
    class function GetMaximumGutterWidth: Integer; static;
    {$ENDREGION}
    {$REGION 'class property gettors and settors'}
    class function GetMinimumPlusSignWidth: Integer; static;
    class function GetScaledColorRectWidth: Integer; static;
    class procedure SetMaximumGutterWidth(const AValue: Integer); static;
    class procedure SetMinimumPlusSignWidth(const AValue: Integer); static;
    class procedure SetScaledColorRectWidth(const AValue: Integer); static;
    {$ENDREGION}
  public
    /// <summary>
  ...
  public
    ...
    /// </summary>
    class property ScaledColorRectWidth: Integer read GetScaledColorRectWidth
      write SetScaledColorRectWidth;
    /// <summary>
    ///   Minimum width of the plus sign indicating whether a child list has
    ///   been expanded or not.
    /// </summary>
    class property MinimumPlusSignWidth: Integer read GetMinimumPlusSignWidth write
        SetMinimumPlusSignWidth;
    /// <summary>
    ///   Maximum gutter width.
    /// </summary>
    class property MaximumGutterWidth: Integer read GetMaximumGutterWidth write SetMaximumGutterWidth;
  end;
...
class function TzObjectInspectorValueManager.GetScaledColorRectWidth: Integer;
begin
  ...
  begin
    if Assigned(parentForm) then
      for I := 0 to parentForm.ComponentCount - 1 do
        if IsClassDerivedFromClass(parentForm.Components[I].ClassType,
          TRttiInstanceType(AItem.AssociatedProperty.PropertyType).MetaclassType) then
...
class function TzObjectInspectorValueManager.GetMaximumGutterWidth: Integer;
begin
  Result := FMaximumGutterWidth;
end;

class function TzObjectInspectorValueManager.GetMinimumPlusSignWidth: Integer;
begin
...
begin
  ...
  begin
    ...
    while ((not Result) and Assigned(rttiType)) do
    begin
...
class procedure TzObjectInspectorValueManager.RegisterObjectEditor(const AObjectClass: TClass;
  AEditorClass: TComponentClass);
begin
  if not Assigned(FEditors) then
    FEditors := TDictionary<Pointer, TComponentClass>.Create;
...
class procedure TzObjectInspectorValueManager.SetMaximumGutterWidth(const AValue: Integer);
begin
  FMaximumGutterWidth := AValue;
end;

class procedure TzObjectInspectorValueManager.SetMinimumPlusSignWidth(const AValue: Integer);
begin
  FMinimumPlusSignWidth := AValue;
end;

class procedure TzObjectInspectorValueManager.SetScaledColorRectWidth(const AValue: Integer);
begin
